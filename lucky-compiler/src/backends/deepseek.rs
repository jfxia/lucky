...
impl DeepSeekBackend {
    ...
    fn build_request_body(
        ...
        format!(
            r#"{{"model":"deepseek-chat","messages":[{}],"temperature":{},"max_tokens":{},"stream":{}}}"#,
    ...
    fn parse_response(body: &str) -> Result<String, String> {
        ...
        let content = extract_json_string_nested(body, "content");
        content.ok_or_else(|| {
            format!(
                "DeepSeek: no content in response. Body: {}",
...
impl Backend for DeepSeekBackend {
    ...
    fn complete(&self, prompt: &str, options: &CompleteOptions) -> Result<String, String> {
        if self.api_key.is_empty() {
            return Err(
                "DEEPSEEK_API_KEY environment variable not set. Set it to your DeepSeek API key."
                    .to_string(),
            );
        }
        ...
        match status {
            ...
            401 => Err("DeepSeek API: authentication failed (401). Check your DEEPSEEK_API_KEY.".into()),
            429 => Err("DeepSeek API: rate limited (429). Wait and retry.".into()),
...
fn http_post_tls(host: &str, port: u16, path: &str, api_key: &str, body: &str) -> Result<String, String> {
    let mut stream = TlsStream::connect(host, port)?;
    send_http_request(&mut stream, host, path, api_key, body)?;
    read_http_response(&mut stream)
}

fn http_post_plain(host: &str, port: u16, path: &str, api_key: &str, body: &str) -> Result<String, String> {
